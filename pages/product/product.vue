<template>
	<view class="container" id="productPage" v-if="is_show">
			...
			<view class="ctn">
				<view class="tabs-box" style="background: none; width: auto;" :style="!showRing ? '' : 'display:none;'">
					<view class="" style="width: 100%;height: 100%;">
						<view class="hx-tabs">
							<view class="hx-tabs-item" style="width: 120px;" v-for="(item,i) in tabs" :key="i" :class="{'hx-tabs-active': swiperCurrent == i}" @click="swiperChange(i,item.id)" >
								<text>{{item.name}}</text>
							</view>
...
<!--					</view>-->
		...
		<view class="carousel">
			<swiper indicator-dots circular=true duration="400">
				<swiper-item class="swiper-item" v-for="(item,index) in goodsInfoData.banners_list" :key="index" @click="previewImg(item,goodsInfoData.banners_list)">
		...
		<view class="container-box introduce-section" >
			...
			<view class="tag-box">
				<block v-for="(item,i) in goodsInfoData.tags" :key="i">
					<text class="tag-item" :style="{ backgroundColor: item.b_color,color:item.color  }">{{item.name}}</text>
				</block>
			...
			<view class="bot-row">
<!--        月销-->
				<text>Ventas mensuales:{{ goodsInfoData.monthly_sales ? goodsInfoData.monthly_sales : 0 }}</text>
				 <text>Stock: {{goodsInfoData.inventory}}</text>
<!--				<text>浏览量: 768</text>-->
			</view>
		...
		<view class="container-box shopping-cart" :class="{'barBorder':showBorder}">
			<view class="price-box" >
				...
				<text class="price">{{ goodsInfoData.real_price }}</text>
				<block v-if="goodsInfoData.price != 0 && goodsInfoData.price > goodsInfoData.real_price">
					<text class="m-price" >{{ goodsInfoData.price }}</text>
					<text class="coupon-tip" >{{(goodsInfoData.real_price/goodsInfoData.price * 100).toFixed(0)}}%de descuento</text>
				</block>
			</view>
			...
			<block v-else>
				...
				<block v-if="!currentGoodsData.number">
					<view class="add-btn" @click="formFirstAddGoods('#eleAdd',currentGoodsData)">
						<i class="hxicon-add" id="eleAdd"></i>
				...
				<block v-else>
					<view class="btn-box">
						...
						<hx-number-box   @lessChange="delChange"  :value="currentGoodsData.number" :rowData="currentGoodsData" :clickTime="animaTime" @addChange="formFirstAddGoods('#eleAdd',currentGoodsData)"></hx-number-box>
					</view>
				</block>
			</block>
		</view>
		...
		<view class="share-section" @click="share">
			<view class="share-icon">
				<text class="hxicon-favor_fill_light"></text>
			</view>
			<text class="tit"></text>
		...
		<view class="container-box">
			<view class="tabs-box" >
				<view class="" style="width: 500px;height: 100%;">
						
					<view class="hx-tabs">
						<view class="hx-tabs-item" style="width: 120px!important;" v-for="(item,i) in tabs" :key="i" :class="{'hx-tabs-active': swiperCurrent == i}" @click="swiperChange(i,item.id)" >
							<text>{{item.name}}</text>
						</view>
						<view class="hx-tabs-slider-box" :style="{left:swiperCurrentSliderLeft + 'px'}">
							<view class="hx-tabs-slider"></view>
        ...
        <view class="info-item">
          <text >{{goodsInfoData.jy_describe}}</text>
        </view>
		...
		<view class="container-box eva-section" id="eva">
			<view class="e-header">
				...
				<text class="tip">{{goodsEvaData.sum ? goodsEvaData.sum : 0}} reseñas</text>
				<text class="hxicon-right"></text>
			</view> 
		...
		<view class="detail-desc" id="coll">
			...
			<rich-text class="detail-desc-content" :nodes="goodsInfoData.describe"></rich-text>
		</view>
		<!-- 购物车 -->
		<view class="foot"  @touchmove.stop.prevent="mpClear" :style="{height: footHeight}" v-if="showFoot">
            ...
            Pagar ahora
					...
					<view class="pscj hx-txt-10 hx-color-gray" v-else>
						<text v-if="storeData.min_order_amount>0">Faltan:{{-(goodsTotalPrice-storeData.min_order_amount)}} </text>
					</view>
					...
					<view class="" style="flex: 1;">
						<scroll-view scroll-y="true" class="goods-list-scroll" :scroll-top="carGoodsScrollTop">
							<view class="goods-list">
								<view class="box" v-for="rowData in shoppCart" :key="rowData.id" v-if="rowData.num>0">
									<view class="m-store-item">
										<view class="m-img">
											<image style="width: 100%;height: 100%;" :src="rowData.image" mode="aspectFit"></image>
										</view>
										<view class="m-text">
											<view class="m-title">
												{{rowData.name}}
											...
											<view class="m-descripe">
												...
												{{rowData.current_form ? rowData.form.name + "：" + rowData.current_form.name : rowData.jy_describe}}
											...
											<view :class="'addEle2_' + rowData.id" class="jumpPosition">
												...
												<hx-number-box  @lessChange="delChange"  :value="rowData.num" :rowData="rowData" :clickTime="400" @addChange="formFirstAddGoods('.addEle2_' + rowData.id,rowData)"></hx-number-box>
...
<script>
  ...
  import {productDetalles} from "../../common/vmeitime-http/request";
  import {
    mapState
  } from 'vuex';
    ...
    computed: {
      ...mapState(['hasLogin','userData'])
		...
		data() {
			return {
				...
				//tabs
				tabs: [
					{name:'Descripción',id: 'detail'},
					{name:'Reseña',id: 'eva'},
					{name:'detalle',id: 'coll'},
				],
				swiperCurrent: 0,
      ...
      let that = this;
      that.is_show=false;
      that.$api.allrequest.productDetalles({id: that.product_id}).then(function (data){
        ...
        that.goodsInfoData =data.data.data.list;//testData.goodsInfo
        that.storeData=data.data.data.shop
        that.is_show=true;
        that.init();
        uni.hideLoading();
      }).catch(function (err){
        ...
        let that=this;
        that.shoppCart=[];
        that.$api.user.cartList().then(function (res){
      ...
      delChange(number,data){
        ...
        that.$api.user.cartDel(sendDate).then(function (res){
          ...
          console.log(e);
			...
			swiperChange(index,idName) {
				...
				this.swiperCurrentSliderLeft= 120 * index;
        ...
        that.$api.user.cartAdd(sendDate).then(function (res){
          ...
          that.afterChangeCar(res.data.data);
          that.touchOnAddGoods('#eleAdd',that.goodsInfoData)
        }).catch(function (e){
          console.log(e);
        })
        if(!this.hasLogin){
      ...
      afterChangeCar(lists){
        ...
        let that = this;
        this.shoppCart=[];
        this.currentGoodsData.number=0;
        var isHaveCurrentGoodsData=false;
        for (let i in lists){
          if(lists[i].shop_id==that.storeData.id){
            that.shoppCart.push(lists[i]);
          }
          //console.log(lists[i],this.goodsInfoData)
          if(lists[i].product_id===this.goodsInfoData.id){
            isHaveCurrentGoodsData=true;
            this.currentGoodsData=lists[i];
            this.currentGoodsData.number=lists[i].num;
          }
        }
        if(!isHaveCurrentGoodsData){
          console.log(this.goodsInfoData);
          this.currentGoodsData.image=this.goodsInfoData.image;
          this.currentGoodsData.shop_id=this.goodsInfoData.shop_id;
          this.currentGoodsData.product_id=this.goodsInfoData.id;
          this.currentGoodsData.num=0;
          this.currentGoodsData.categories_id=this.goodsInfoData.categories_id;
          this.currentGoodsData.name=this.goodsInfoData.name;
          this.currentGoodsData.price=this.goodsInfoData.price;
          this.currentGoodsData.real_price=this.goodsInfoData.real_price;
          this.currentGoodsData.jy_describe=this.goodsInfoData.jy_describe;
          this.currentGoodsData.id=0;
        }
        ...
        for (let i in that.shoppCart){
          that.goodsTotalPrice+=parseFloat(that.shoppCart[i].real_price)*that.shoppCart[i].num;
