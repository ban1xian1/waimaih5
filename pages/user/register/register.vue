    ...
    <view class="content">
      ...
      <view class="main">
        <wInput
            ...
            type="text"
            maxlength="15"
            placeholder="Nombre de usuario (5-15 caracteres)"
        ></wInput>
        <wInput
            v-model="password"
            type="password"
            maxlength="20"
            placeholder="Contraseña (6-20 caracteres)"
            isShowPass
        ></wInput>
        <wInput
            v-model="nombreReal"
      ...
      <view class="footer">
        ...
        <!-- 协议地址 -->
        <text @click="go_login" style="color: #005cbf;font-size: 14px;">Acceso</text>
      </view>
    </view>
  </view>
</template>
...
<script>
import {Reg} from "../../../common/vmeitime-http/user";
    ...
    startReg() {
      ...
      if (this.username.length<5||this.username.length>15) {
        uni.showToast({
          icon: 'none',
          position: 'bottom',
          title: 'Por favor, introduzca una cuenta de 5 a 15 caracteres'
        });
        return false;
      }
      if (this.password.length < 6) {
        uni.showToast({
          icon: 'none',
          position: 'bottom',
          title: 'Ingrese una contraseña de 6 a 20 caracteres'
        });
        return false;
      }
      ...
      if (this.code.length == '') {
        uni.showToast({
          icon: 'none',
          position: 'bottom',
          title: 'El código de invitación es obligatorio'
        });
        return false;
      }
      if (this.nombreReal.length == '') {
        uni.showToast({
          icon: 'none',
          position: 'bottom',
      ...
      let params={
        ...
        nombreReal:that.nombreReal,
        code:that.code,
      }
      this.isRotate=true;
      that.$api.user.Reg(params).then((res)=>
      {
        that.isRotate=false;
        if(res.data.code != '1'){
          uni.showToast({
